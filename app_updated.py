...
def is_schedule_query(text: str) -> bool:
    ...
    # 明示的に「予定/スケジュール」を聞いているケース
    if any(w in t for w in ("予定", "スケジュール", "schedule")):
        return True

    # 「今日なにある？」のように “予定” が省略されるケースを拾う
    if any(w in t for w in SCHEDULE_SCOPE_WORDS) and any(h in t for h in ("何", "なに", "ある", "入ってる", "やること")):
        return True
